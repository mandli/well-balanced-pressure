...
def setplot(plotdata=None):
    ...
    def surge_afteraxes(cd):
        surgeplot.surge_afteraxes(
            cd, track, plot_direction=False, kwargs={"markersize": 4}
        )
    ...
    plotfigure = plotdata.new_plotfigure(
        name="Gauge Surfaces", figno=300, type="each_gauge"
    )
    plotfigure.show = True
    ...
    def gauge_location_afteraxes(cd):
        ...
        gaugetools.plot_gauge_locations(
            cd.plotdata, gaugenos="all", format_string="bx", add_labels=False
        )
        gaugetools.plot_gauge_locations(
            cd.plotdata,
            gaugenos=[0, 11, 22, 10, 21, 32],
            format_string="ko",
            add_labels=True,
        )
