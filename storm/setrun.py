...
class SplittingData(data.ClawData):
    ...
    def __init__(self):
        ...
        self.add_attribute("test_type", 0)
...
def setrun(claw_pkg="geoclaw"):
    ...
    rundata.gaugedata.gauges.append(
        [1, -500e3, 0.0, rundata.clawdata.t0, rundata.clawdata.tfinal]
    )
    rundata.gaugedata.gauges.append(
        [2, 0.0, 0.0, rundata.clawdata.t0, rundata.clawdata.tfinal]
    )
    rundata.gaugedata.gauges.append(
        [3, 500e3, 0.0, rundata.clawdata.t0, rundata.clawdata.tfinal]
    )
...
def setgeo(rundata):
    ...
    num_forecasts = 12
    storm.t = np.linspace(rundata.clawdata.t0, rundata.clawdata.tfinal, num_forecasts)

    def ramp_function(t):
        c = days2seconds(1)
        return np.where(
            t < 0.0, -2 / c**3 * t**3 - 3 / c**2 * t**2 + 1, np.ones(t.shape)
        )

    def storm_x(t):
        """Position of storm eye, based on km/h average for Atlantic basin."""
        return 15.0 / 3.6 * t
