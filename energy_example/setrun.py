...
class SplittingData(data.ClawData):
    r""""""
    def __init__(self):
        super(SplittingData, self).__init__()

        # GeoClaw physics parameters
        self.add_attribute('split_forcing', False)
        self.add_attribute('test_type', 'pressure')

    def write(self, data_source='setrun.py', out_file='splitting.data'):

        self.open_data_file(out_file, data_source)
        self.data_write('split_forcing', description="(Use split forcing)")
        if self.test_type.lower() == "pressure":
            self.test_type = 1
        elif self.test_type.lower() == 'bathymetry':
            self.test_type = 2
        else:
            self.test_type = 0
        self.data_write('test_type', description="(Pressure or bathy test)")
        self.close_data_file()


class EnergyData(data.ClawData):
    r""""""

    def __init__(self):
        super(EnergyData, self).__init__()

        # GeoClaw physics parameters
        self.add_attribute('U', 1.0)
        self.add_attribute('R_m', 1.0)
        self.add_attribute('dp', 1.0)

    def write(self, data_source='setrun.py', out_file='energy.data'):
...
def setrun(claw_pkg='geoclaw'):
    ...
    # Run setup 2.1, 2.2, 2.3, 2.2 respectively
    Fr = [0.0, 0.5, 1.0, 2.0]

    rundata.add_data(EnergyData(), 'energy_data')
    # Fr = U / sqrt(geo_data.gravity * d)
    T_0 = R_m / np.sqrt(rundata.geo_data.gravity * d)
    rundata.energy_data.U = Fr[2] * np.sqrt(rundata.geo_data.gravity * d)
    rundata.energy_data.R_m = R_m
    rundata.energy_data.dp = eta_0 * rundata.geo_data.rho * rundata.geo_data.gravity

    rundata.topo_data.basin_depth = -d
    rundata.topo_data.shelf_depth = -d
    ...
    buffer_factor = 30
    length_factor = 600
    clawdata.lower[0] = -buffer_factor * R_m
    clawdata.upper[0] = length_factor * R_m

    clawdata.lower[1] = -buffer_factor * R_m
    clawdata.upper[1] = buffer_factor * R_m

    # Number of grid cells:
    clawdata.num_cells[0] = int((clawdata.upper[0] - clawdata.lower[0]) / 1e3)
    clawdata.num_cells[1] = int((clawdata.upper[1] - clawdata.lower[1]) / 1e3)
    ...
    # -------------
    clawdata.t0 = 0.0
    clawdata.tfinal = length_factor * T_0
    ...
    if clawdata.output_style == 1:
        ...
        clawdata.output_times = np.array([0, 1, 2, 3, 4, 5, 10, 20, 30, 40, 50]) * T_0
        clawdata.tfinal = clawdata.output_times[-1]
