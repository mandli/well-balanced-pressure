...
subroutine setaux(mbc,mx,my,xlow,ylow,dx,dy,maux,aux)
    ...
    use geoclaw_module, only: sea_level, ambient_pressure, rho, grav
    ...
    if (test_topography > 0) then
        ! Add topography to the source term directly if using the split approach
        ! for bathymetry
        if (test_type == 2) then
            do jj = 1 - mbc, my + mbc
                ym = ylower + (jlo+jj-1.d0) * dy
                ...
                do ii = 1 - mbc, mx + mbc
                    ...
                    if (abs(x - 0.5d0) < 0.1d0) then
                        aux(1,ii,jj) = -1.d0 + 0.25d0 * (cos(PI * (x - 0.5d0) / 0.1d0) + 1d0)
                    else
                        aux(1,ii,jj) = -1.d0
                    end if
                end do
